...
def format_data(data):
    ...
    return text_input, text_lengths, mel_input, mel_lengths, linear_input, stop_targets, speaker_ids, avg_text_length, avg_spec_length,wav
...
def train(model, criterion, criterion_st, optimizer, optimizer_st, scheduler,
    ...
    for num_iter, data in enumerate(data_loader):
        ...
        # format data
        text_input, text_lengths, mel_input, mel_lengths, linear_input, stop_targets, speaker_ids, avg_text_length, avg_spec_length,wav = format_data(data)
        ...
        if c.bidirectional_decoder:
            ...
            decoder_output, postnet_output, alignments, stop_tokens, mu, logvar, z = model(text_input, text_lengths, mel_input, speaker_ids=speaker_ids,ref_cond=True)
            #print('>>>>>>>>>>>>>>>>>>>>>>>>4')
        # loss computation
        ...
        if args.rank == 0:
            ...
            if global_step % c.save_step == 0:
                ...
                figures = {
                    ...
                    "ref_audio": plot_spectrogram(gt_spec, ap),
                ...
                #tb_logger.tb_train_audios(global_step,{'AudioRef': wav[0] },c.audio["sample_rate"])
        end_time = time.time()
...
def evaluate(model, criterion, criterion_st, ap, global_step, epoch):
    ...
    with torch.no_grad():
        if data_loader is not None:
            for num_iter, data in enumerate(data_loader):
                ...
                # format data
                text_input, text_lengths, mel_input, mel_lengths, linear_input, stop_targets, speaker_ids, _, _ ,wav= format_data(data)
            ...
            if args.rank == 0:
                ...
                tb_logger.tb_eval_audios(global_step, {"RefAudio": wav[idx]}, c.audio["sample_rate"])
    ...
    if args.rank == 0 and epoch==-1: # >= c.test_delay_epochs:
        if c.test_sentences_file is None:
            test_sentences = ["I have seen him wringing his hands after such a rebuff and I am sure the annoyance and the terror he lived in must have greatly hastened his early and unhappy death."]
            '''
        ...
        for idx, test_sentence in enumerate(test_sentences):
            try:
                wav,wav_noRef, alignment, decoder_output, postnet_output,postnet_output_noRef, stop_tokens = synthesis(
                    model,
                    test_sentence,
                    c,
                    use_cuda,
                ...
                #ap.save_wav(wav, '/home/huda/desktop/test.wav')
                test_audios['{}-audio'.format(idx)] = wav
                test_audios['{}-audio_noRef'.format(idx)] = wav_noRef                  
                test_figures['{}-prediction'.format(idx)] = plot_spectrogram(postnet_output, ap)
                test_figures['{}-prediction_noRef'.format(idx)] = plot_spectrogram(postnet_output_noRef, ap)
                test_figures['{}-alignment'.format(idx)] = plot_alignment(alignment)
            except:
                print(" !! Error creating Test Sentence -", idx)
                traceback.print_exc()
        style_audio=ap.load_wav(style_wav)

        test_audios['Ref_audio'] = style_audio                  
...
def main(args):  # pylint: disable=redefined-outer-name
    ...
    for epoch in range(0, c.epochs):
        ...
        train_loss, global_step = train(model, criterion, criterion_st,optimizer, optimizer_st, scheduler, ap, global_step, epoch)
